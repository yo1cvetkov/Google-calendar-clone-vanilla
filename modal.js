...
function getEventFormModalBody(event, saveCallback, deleteCallback) {
  ...
  formModalBody.querySelector("[data-title]").textContent = isNewEvent ? "Add Event" : "Edit  Event";
  formModalBody.querySelector("[data-date]").textContent = format(event.date, "M/d/yyyy");

  const form = formModalBody.querySelector("[data-form]");
  form.querySelector("[data-save-btn]").textContent = isNewEvent ? "Add" : "Update";
