...
export default function createEventElement(event) {
  const element = event.isAllDay ? createAllDayEventElement(event) : createTimedEventElement(event);
...
function createTimedEventElement(event) {
  ...
  element.querySelector("[data-time]").textContent = format(parse(event.startTime, "HH:mm", event.date), "h:mmaaa");
